...
! TEST
integer :: Nt
real(wp), allocatable :: delay_res_PINEM(:,:), delay_res_Gauss(:,:), delay_times(:)
...
N_states = 14  ! 1s, 2s, 2p-1, 2p0, 2p1, 3s, 3p-1, 3p0, 3p1, 3d-2, 3d-1, 3d0, 3d1, 3d2
allocate(state_array(N_states))
allocate(state_coeffs(N_states))
if (array_cal_index > N_states) then 
    write(*,*) 'Command line input larger than number of states!'
    stop
end if 
...
state_coeffs = [1._wp/sqrt(2._wp), 0._wp, 1._wp/sqrt(4._wp), 0._wp, -1._wp/sqrt(4._wp), &
                0._wp, 0._wp, 0._wp, 0._wp, 0._wp, 0._wp, 0._wp, 0._wp, 0._wp]  ! Bottom row is n=3 states 

! Setup states in terms of slater orbitals 
! state, Ns, c_arr, alpha_arr, n_arr, l, m, E
...
! n=3 states from here 
call init_slater(state_array(6), 3, [2._wp/sqrt(27._wp), -2._wp/sqrt(27._wp)*2._wp/3._wp, 2._wp/sqrt(27._wp)*2._wp/27._wp], &
                [1._wp/3._wp, 1._wp/3._wp, 1._wp/3._wp], [1, 2, 3], 0, 0, -0.5_wp/3._wp**2)  ! 3s 

call init_slater(state_array(7), 2, [4._wp*sqrt(2._wp)/9._wp/sqrt(27._wp), -4._wp*sqrt(2._wp)/9._wp/sqrt(27._wp)/6._wp], &
                [1._wp/3._wp, 1._wp/3._wp], [2, 3], 1, -1, -0.5_wp/3._wp**2)  ! 3p-1

state_array(8) = state_array(7)
state_array(8)%m = 0
state_array(9) = state_array(7)
state_array(9)%m = 1

call init_slater(state_array(10), 1, [4._wp/(27._wp*sqrt(270._wp))], [1._wp/3._wp], [3], 2, -2, -0.5_wp/3._wp**2)  ! 3d-2
state_array(11) = state_array(10)
state_array(11)%m = -1
state_array(12) = state_array(10)
state_array(12)%m = 0
state_array(13) = state_array(10)
state_array(13)%m = 1
state_array(14) = state_array(10)
state_array(14)%m = 2
...
do i=1, N_states
    state_array(i)%init_coeff = state_array(i)%init_coeff * exp(cmplx(0._wp, -state_array(i)%E*ta0, wp))
end do 
...
write(*,*) 'Performing calculations for final channel ', array_cal_index 
res = angular_dist_channel(k_list, state_array(array_cal_index), state_array, Wp_p, theta_final, phi_final) 
write(save_file, "(A,I0,A)") 'data/angular_dist_channel_', array_cal_index, '.dat'
call save_array(save_file, res)

! Also save the dimensions of the arrays in a .txt file (if this is channel 1 calculation)
if (array_cal_index == 3) then
...
function PINEM_wp(k, theta, Wp_p) result(res)
    ...
    res = bessel_jn(0, WP_p%g_factor) * exp(cmplx(0._wp, phi_N, wp)) * Gaussian_one_dim(kz, Wp_p%k0z, Wp_p%sigma_z)
    do N=1, max_N 
        res = res + bessel_jn(N, Wp_p%g_factor) * exp(cmplx(0._wp, phi_N, wp)) * Gaussian_one_dim(kz, Wp_p%k0z + N*delta, Wp_p%sigma_z)
        res = res + bessel_jn(-N, Wp_p%g_factor) * exp(cmplx(0._wp, phi_N, wp)) * Gaussian_one_dim(kz, Wp_p%k0z - N*delta, Wp_p%sigma_z)
    end do 
