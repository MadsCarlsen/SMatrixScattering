...
write(*,*) sigma_z, sigma_perp, omega
...
if (cal_PINEM) then 
    if (cal_tot_elastic) then 
        ...
        call save_array('data/PINEM_tot_elastic.dat', res)
    end if 
    ...
    if (cal_scatt_elastic) then 
        ...
        .true., .true.)
        call save_array('data/PINEM_scatt_elastic.dat', res)
    end if 
    ...
    if (cal_scatt_inelastic) then 
        ...
        .true., .false.)
        call save_array('data/PINEM_scatt_inelastic.dat', res)
    end if
    ...
    if (cal_interference) then 
        ...
        call save_array('data/PINEM_interference.dat', res)
    end if 
end if 
...
if (cal_Gauss) then 
    if (cal_tot_elastic) then 
        ...
        call save_array('data/Gauss_tot_elastic.dat', res)
    end if 
    ...
    if (cal_scatt_elastic) then 
        ...
        res = angular_dist_scattered(k_list, T00, theta_final, phi_final, theta_int, phi_int, tz, t_perp, z0, b, omega, &
        .false., .true.)
        call save_array('data/Gauss_scatt_elastic.dat', res)
    end if 
    ...
    if (cal_scatt_inelastic) then 
        ...
        res = angular_dist_scattered(k_list, T01, theta_final, phi_final, theta_int, phi_int, tz, t_perp, z0, b, omega, &
        .false., .false.)
        call save_array('data/Gauss_scatt_inelastic.dat', res)
    end if
    ...
    if (cal_interference) then 
        ...
        call save_array('data/Gauss_interference.dat', res)
    end if 
end if 
...
function angular_dist_elastic(k_list, T_data, theta_final, phi_final, theta_int, phi_int, tz, t_perp, z0, b, omega, PINEM_bool) 
    ...
    do i=1, size(theta_final)
        ...
        do j=1, size(phi_final)
            ...
            angular_dist_elastic(i,j) = trapz(k_list**2 * abs(S_list)**2, k_list(2)-k_list(1)) 
        end do 
    end do 
    !$OMP END PARALLEL DO
